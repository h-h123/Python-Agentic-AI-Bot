...
def calculate_discounted_price(price, discount):
    ...
    try:
        ...
        return float(price * (Decimal('1') - (discount / Decimal('100')))) if discount > 0 else float(price)
    except (ValueError, TypeError):
        return float(price)
...
def format_date(date):
    ...
    if not date:
        return ""
    return date.strftime("%B %d, %Y") if isinstance(date, datetime) else date
...
def generate_unique_filename(original_filename):
    ...
    return f"{name}_{unique_part}{ext}"